...
export async function listPromptPlaza(params: {
  ...
  const response = await apiClient.get<PaginatedResult<PromptPlazaItem>>("/prompts", {
    params: {
      page: params.page ?? 1,
...
export async function submitPromptPlaza(payload: {
  ...
  try {
    const response = await apiClient.post<{ uuid: string; status: string }>("/prompts", payload);
    return response.data;
  } catch (error) {
...
export async function listMyPromptSubmissions() {
  const response = await apiClient.get<Array<PromptPlazaItem & { status: string; rejectReason: string | null }>>(
    "/prompts/mine"
  );
  return response.data;
}
...
export async function togglePromptLike(uuid: string) {
  const response = await apiClient.post<{ liked: boolean }>(`/prompts/${uuid}/like`);
  return response.data;
}

export async function downloadPrompt(uuid: string) {
  const response = await apiClient.post<PromptPlazaDetail>(`/prompts/${uuid}/download`);
  return response.data;
}
...
export async function listStoryPlaza(params: {
  ...
  const response = await apiClient.get<PaginatedResult<StoryPlazaItem>>("/stories", {
    params: {
      page: params.page ?? 1,
      limit: params.limit ?? 20,
      sort: params.sort ?? "newest",
...
export async function submitStoryPlaza(payload: {
  ...
  try {
    const response = await apiClient.post<{ uuid: string; status: string }>("/stories", payload);
    return response.data;
  } catch (error) {
...
export async function listMyStorySubmissions() {
  const response = await apiClient.get<Array<StoryPlazaItem & { status: string; rejectReason: string | null }>>(
    "/stories/mine"
  );
  return response.data;
}
...
export async function toggleStoryLike(uuid: string) {
  const response = await apiClient.post<{ liked: boolean }>(`/stories/${uuid}/like`);
  return response.data;
}

export async function downloadStory(uuid: string) {
  const response = await apiClient.post<StoryPlazaDetail>(`/stories/${uuid}/download`);
  return response.data;
}
