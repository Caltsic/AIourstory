...
import { downloadPrompt, getPromptPlazaDetail, togglePromptLike } from "@/lib/plaza-api";
import { generatePresetId, savePreset, setActivePresetId } from "@/lib/prompt-store";
...
export default function PromptDetailScreen() {
  ...
  async function handleUsePreset() {
    ...
    try {
      ...
      Alert.alert("成功", "已保存到本地并激活。", [{ text: "去查看", onPress: () => router.replace("/(tabs)/prompts") }]);
    } catch (err) {
  ...
  return (
    <ScreenContainer>
      <View style={[styles.header, { borderBottomColor: colors.border }]}> 
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>提示词详情</Text>
        <View style={{ width: 32 }} />
      </View>
      ...
      {item ? (
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <Text style={{ color: colors.muted }}>{item.description || "无描述"}</Text>
          <Text style={[styles.meta, { color: colors.muted }]}>by {item.author.nickname} · 下载 {item.downloadCount}</Text>

          <View style={styles.buttonRow}>
            <TouchableOpacity style={[styles.secondaryBtn, { borderColor: colors.border }]} onPress={handleLike}>
              <Text style={{ color: colors.foreground }}>{item.isLiked ? "取消点赞" : "点赞"} ({item.likeCount})</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.primaryBtn, { backgroundColor: colors.primary }]} onPress={handleUsePreset}>
              <Text style={{ color: "#fff", fontWeight: "700" }}>使用此预设</Text>
            </TouchableOpacity>
          </View>

          {!auth.isBound ? (
            <TouchableOpacity onPress={() => router.push("/login" as any)}>
              <Text style={{ color: colors.primary, marginBottom: 12 }}>绑定账号后可投稿与管理</Text>
            </TouchableOpacity>
          ) : null}

          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>提示词内容</Text>
          {promptEntries.map(([key, value]) => (
            <View key={key} style={[styles.block, { borderColor: colors.border, backgroundColor: colors.surface }]}>
              <Text style={{ color: colors.foreground, fontWeight: "700", marginBottom: 6 }}>{key}</Text>
